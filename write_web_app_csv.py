...
def assign_topics(df, modle_file, vector_file):
    vectorizer, vectors = pkl.load(open(vector_file, "rb"))
...
if __name__=='__main__':
    ...
    df = assign_topics(df, modle_file='data/model_'+str(n_topics)+'.pkl',
                           vector_file='data/vectorizer.pkl')

    articles_per_topic(df, "topic_browser/static/articles_per_topic.csv")
    newdf = creat_data_csv(df)
    newdf.to_csv("topic_browser/static/data.csv", index=False)
